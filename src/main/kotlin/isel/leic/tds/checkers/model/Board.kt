 ...
 */
private fun BoardRun.retrieveValidSquaresToMoveTo(
    fromSqr: Square,
...
private fun BoardRun.getAvalaibleCaptures(
    ...
    val turnMoves = if (previousCaptureSqr != null) {
        ...
        moves.filter { it.value.player === turn }.also{ println("filter result $it")}
    }
    // Search every checker for a valid capture
    turnMoves.forEach { (sqr, checker) ->
        val x = findOpponentCheckersInAllDiagonals(centerSqr = sqr, checker as Checker)
        println("after return: $x")
            x.forEach { dSqr -> // For every diagonal square in the list:
                // Find a path to land
...
private fun BoardRun.findOpponentCheckersInAllDiagonals(
    ...
    when (checker) {
        is Piece -> {
            ...
            println("diagonals of $centerSqr: ${centerSqr.adjacentDiagonalsList}")
            val x = centerSqr.adjacentDiagonalsList
                .filter { isOtherPlayerChecker(it) }
            println("before return: $x")
            return x
        }
        is King -> {
            // Search all diagonals to find a square with a checker
            val x = listOfNotNull(
                centerSqr.upperBackSlash.find { isOtherPlayerChecker(it) },
                centerSqr.upperSlash.find { isOtherPlayerChecker(it) },
                centerSqr.lowerBackSlash.find { isOtherPlayerChecker(it) },
                centerSqr.lowerSlash.find { isOtherPlayerChecker((it)) }
            )
            println("not me: $x")
            return x
        }
...
fun BoardRun.isOtherPlayerChecker(sqr: Square): Boolean {
    println("current moves: $moves")
...
private fun BoardRun.checkWin(): Boolean {
    ...
    println("Start check win")
    println("Other turn: ${turn.other()}")
    if (getAvalaibleCaptures(null, turn.other()).isNotEmpty()) {
        return false
    }
    println("Ended check win")
    moves
