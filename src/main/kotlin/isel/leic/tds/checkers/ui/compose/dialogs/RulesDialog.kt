...
import isel.leic.tds.checkers.ui.compose.ExpandableCard
...
fun RulesDialog(onCancel: ()->Unit) = Dialog(
    ...
    title = "Rules",
    state = DialogState(width = 300.dp, height = 450.dp),
    resizable = false,
...
val rulesList = listOf(
    ...
    Rule("Only when a piece is in a close diagonal of an opponent's checker " +
            ...
            "can be made. The opponent's checker gets eliminated after a successful capture."),
    ...
    Rule("A capture must be made if one is available to continue with the game. " +
